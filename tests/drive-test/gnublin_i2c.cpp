...
int gnublin_i2c::open_fd() 
{
	...
	if ((fd = open(devicefile.c_str(), O_RDWR)) < 0) 
        ...
        int r;
	if ( (r = ioctl(fd, I2C_SLAVE, slave_address)) < 0) 
          {
            perror("failed to set target address");
            return r;
          }
...
int gnublin_i2c::receive(unsigned char *RxBuf, int length){
	...
	if (read(fd, RxBuf, length) != length)
          return fprintf(stderr, "i2c read error! Address: %d; dev file: %s\n", slave_address, devicefile.c_str());
